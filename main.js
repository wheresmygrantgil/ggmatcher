...
const loadedLibs = { dt: false, chart: false };
...
async function ensureCharts() {
  if (loadedLibs.chart) return;
  await loadScript('https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js');
  loadedLibs.chart = true;
}

async function ensureDataTables() {
  if (loadedLibs.dt) return;
  ...
  await Promise.all([
    ...
    loadStyle('https://cdn.datatables.net/buttons/2.4.1/css/buttons.dataTables.min.css'),
    loadStyle('https://cdn.datatables.net/colreorder/1.6.2/css/colReorder.dataTables.min.css'),
    loadStyle('https://cdn.datatables.net/responsive/2.5.0/css/responsive.dataTables.min.css')
  ]);
  await Promise.all([
    ...
    loadScript('https://cdn.datatables.net/plug-ins/1.13.6/features/mark.js/datatables.mark.js')
  ]);
  loadedLibs.dt = true;
}
...
function showTab(name) {
  ...
  if (name === 'stats') {
    ...
    ensureCharts().then(() => requestAnimationFrame(showDashboard));
